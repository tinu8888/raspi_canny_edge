...
void convertToPGM(const cv::Mat& gray, unsigned char* pgm) {
    int index = 0;
    for (int i = 0; i < gray.rows; ++i) {
        for (int j = 0; j < gray.cols; ++j) {
            pgm[index++] = (unsigned char)gray.at<uchar>(i, j);
        }
    }
}
...
int main(int argc, char* argv[])
{
   ...
   while (true) {
       ...
       convertToPGM(grayFrame, image);

       canny(image, ROWS, COLS, SIGMA, TLOW, THIGH, edge);

       // Convert edge unsigned char array back to cv::Mat for imshow
       cv::Mat edgeFrame(ROWS, COLS, CV_8UC1, edge);

       // Display the edge image
       cv::imshow("Edge Image", edgeFrame);

       i++;

       if (cv::waitKey(30) == 27) {
           std::cout << "ESC key is pressed by the user. Stopping the camera." << std::endl;
           break;
       }
   }
    ...
    unsigned char *result)
{
   ...
   int mag1,mag2; int xperp=0;int yperp=0;
